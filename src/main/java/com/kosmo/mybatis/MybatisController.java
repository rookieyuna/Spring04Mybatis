...
public class MybatisController {
	...
	public String list(Model model, HttpServletRequest req) {
		...
		//페이지 처리를 위한 설정값
		int pageSize = 4; //한페이지당 출력할 게시물의 개수
		int blockPage = 2; //한블럭당 출력할 페이지번호의 개수
		
		/*
		 //전체페이지 수 계산 int totalPage =
		 (int)(Math.ceil(((double)totalRecordCount/pageSize)));
		 */
		
		//현재페이지 번호 설정
		/*
		방명록URL?nowPage=		▶ 페이지번호 빈값
		방명록URL?nowPage=10	▶ 페이지번호 10으로설정
		방명록URL				▶ null로 판단
		 */
		...
		//해당페이지에 출력할 게시물의 구간을 계산한다.
		int start = (nowPage-1) * pageSize + 1;
		int end =nowPage * pageSize;
		
		/*
		서비스 역할을 하는 인터페이스의 추상메서드를 호출하면 mapper가 동작됨
	...
	public String listSearch(Model model, HttpServletRequest req) {
		...
		//Mapper로 전달할 파라미터 저장을 위한 DTO객체 생성
		...
		//위에서 계산한 게시물 구간 start, end를 DTO에 저장
	...
	public String write(Model model, HttpSession session, HttpServletRequest req) {
		...
		/*
		매핑된 메서드 내에서 session 내장객체를 사용하기 위해
		매개변수로 선언해준다.
		 */
		//session영역에 해당 속성이 없다면 로그아웃 상태이므로 로그인페이지로 이동한다.
	...
	public ModelAndView loginActioin(
		...
		//글쓰기 페이지로의 진입에 실패한 경우라면 backUrl을 통해 글쓰기 페이지로 이동
	...
	public String writeAction(Model model, HttpServletRequest req, HttpSession session) {
		...
		글쓰기 처리에서도 반드시 세션을 학인한 후 처리해야 한다.
		 */
		if(session.getAttribute("siteUserInfo")==null) {
			//만약 세션이 끊어졌다면 로그인페이지로 이동
			return "redirect:login.do";
		}
		
		//Mybatis 사용
		//insert문을 싱해시 입력에 성공한 해의 개수가 정수형으로 반환
